...
class IllegalCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "Bisaya mag code ampota, bawal yan: ", details
        )
...
KEYWORDS = ["SET", "ALSO", "OR", "NO", "IF", "THEN", "ELIF", "ELSE"]  # AND  # OR  # NOT
...
class Parser:
    ...
    def power(self):
        return self.bin_op(self.atom, (TT_POW,), self.power)
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+' , '-', '(', 'NO' ",
                )
            )
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.raised_by(right)
